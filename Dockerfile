...
# Install Node.js (for Alpine)
RUN apk add --no-cache nodejs npm

COPY . /source

# Build frontend
WORKDIR /source/performancemanagementchart.client
RUN npm install && npm run build

# Copy frontend build to ASP.NET wwwroot
WORKDIR /source/PerformanceManagementChart.Server
RUN rm -rf wwwroot && mkdir wwwroot
RUN cp -r /source/performancemanagementchart.client/dist/* wwwroot/
...
ENTRYPOINT ["dotnet", "PerformanceManagementChart.Server.dll"]
